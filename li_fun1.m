function log_likelihood = li_fun1(a,x1,y1,t1,X,Y,T,S,p,q)
x_1 = S(1);%Rectangular boundaries, translated to the origin 
x_2 = S(2);
y_1 = S(3);
y_2 = S(4);
t_1 = t1(1);
...
log_likelihood = -(sum(log(a(1:p)*mu + a(5*p+1:5*p+q)*G))...
    -(a(1:p)*A) -sum(a(5*p+1:5*p+q)*(B.*H)));%The fmincon function is used to find the minimum value, so a negative sign is added here.